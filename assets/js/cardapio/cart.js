...
function cartData() {
    ...
    for (const i in cart) {
        ...
        for (let pa = 0; pa < cart[i].aditional.length; pa++) {
            totalca += parseInt(cart[i].aditional[pa].preco * cart[i].quantityProduct);  
        }
        cartItem += `
        ...
        <div class="cart-item shadow" style="display:block;margin-top: -25px;border-top-left-radius: 0;border-top-right-radius: 0;">
            <div>
                ...
                ${cart[i].aditional.map((ad, index) => `
                    <li style="list-style: none;margin: 10px 15px;">
                        <span style="display:flex;justify-content: space-between;">
                            ...
                            <div><span>${ad.item}</span></div>
                            <div><span>${real(parseInt(ad.preco * cart[i].quantityProduct))}</span><button class="btn-del-ad" onclick="delAd(${i},${index})">x</button></div>
                            </div>
                ...
                <div style="display: flex;justify-content: space-between;align-items: center;margin: 15px 10px;">
                    <span class="p-c-item" onclick="OpenAdicionais(${cart[i].idProduct}, ${i})">Mais Acréscimos<span class="material-icons">touch_app</span></span>
...
function updateCart() {
    ...
    for (const ct in cart) {
        ...
        for (const ad in cart[ct].aditional) {
            total_adicionais += parseInt(cart[ct].aditional[ad].preco * cart[ct].quantityProduct);
        }
...
function checkout() {
    ...
    $.ajax({
        type: "POST",
        url:   'https://descomplicazap.com/new/order',
        data: {
            companie,
            company_object,
            order_object,
        ...
        success: function(res) {
            if(res.code === 0) {
                ...
                msg += `https://descomplicazap.com/meupedido/${res.pedido}`;
