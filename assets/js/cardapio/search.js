...
function seachProducts(value, company) {
  ...
  if (value.length < 1) {
    ...
    $.ajax({
      type: "POST",
      url: "https://descomplicazap.com/search",
      ...
      success: function (res) {
        ...
        for (const s in res.search) {
          html += `
          <div class="card-c shadow" onclick="verifyDay(${res.search[s].id}, ${res.search[s].category})">
              <div class="row-c">
                  <div class="img-c">
                      <img src="${res.search[s].image}" class="img-card" alt="${res.search[s].name_product}">
                  </div>
                  <div class="info-c">
