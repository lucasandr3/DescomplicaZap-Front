...
class Usuarios extends Model
{
	...
	public function verifyLogin($storeName)
	{

			$sql ="SELECT * FROM company WHERE slug = :slug";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':slug', $storeName);
			$sql->execute();
			...
			if ($sql->rowCount() > 0) {
				...
				// $sqlc ="SELECT name FROM company WHERE id = :company";
				// $sqlc = $this->db->prepare($sqlc);
				// $sqlc->bindValue(':company', $data['id_company']);
				// $sqlc->execute();

				// if($sqlc->rowCount() > 0) {
				// 	$company = $sqlc->fetch(\PDO::FETCH_ASSOC);
	...
	public function validateEmailForgot($email)
	{
		$sql ="SELECT * FROM usuarios WHERE email_user = :email_user";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(":email_user", $email);
		$sql->execute();

		if ($sql->rowCount() > 0) {
			$data = $sql->fetch();
			$_SESSION['id_user_reset'] = $data['id_user'];
			return true;
		} else {
			return false;
		}
	...
	public function newPass($password, $id_user)
	{
		...
		$sql ="UPDATE usuarios SET senha = :se WHERE id_user = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':se', $passhash);
